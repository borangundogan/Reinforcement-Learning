...
class GridWorld(object):
    ...
    def __init__(self, m, n, magicSquares):
        ...
        #create possible action space
        self.possibleActionSpace = ["U", "D", "L", "R"]
        
        self.addMagicSquares(magicSquares)
        
        #agent firstly position
    ...
    def actionSpaceSample(self):
        return np.random.choice(self.possibleActionSpace)
...
if __name__ == "__main__":
    ...
    #agent farsighted
    GAMMA = 1.0
    #epslyon greedy selection
    EPS = 1.0
    
    Q = {}
    for state in env.stateSpacePlus:
        for action in env.possibleActionSpace:
            Q[state, action] = 0
    ...
    for i in range(numGames):
        ...
        while not done:
            ...
            action = maxAction(Q, observation, env.possibleActionSpace) if rand< (1 - EPS)\
                else env.actionSpaceSample()
                
            observation_, reward, done, info = env.step(action)
            epRewards += reward
            
            action_ = maxAction(Q, observation_, env.possibleActionSpace)
